...
(function(global) {
    ...
    function Dword() {
        ...
        dword.setMode = function(mode) {
            var is = CodeMirror.modes.hasOwnProperty(mode);
            
            if (mode && !is)
                CodeMirror.requireMode(mode, function() {
                    var reg     = /^(css|json|yaml|javascript|coffeescript)$/,
                        isLint  = reg.test(mode);
                    
                    Ace.setOption('mode', mode);
                    Ace.setOption('gutters', ['CodeMirror-lint-markers']);
                    
                    if (!isLint)
                        Ace.setOption('lint', false);
                    else
                        setJsHintConfig(function(jshint) {
                            Ace.setOption('lint', jshint);
                        });
                });
