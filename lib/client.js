...
(function(global) {
    ...
    function Dword() {
        ...
        dword.addCommand       = function(options) {
        ...
        dword.setUseOfWorker   = function(mode) {
            var isMatch,
                session = dword.getSession(),
        ...
        dword.setMode          = function(mode) {
            Ace.setOption('mode', mode);
            CodeMirror.autoLoadMode(Ace, mode);
        ...
        dword.setModeForPath   = function(name) {
            ...
            addExt(name, function(name) {
                var htmlMode, jsMode, isHTML, isJS,
        ...
        function setEmmet() {
            ...
            if (isEmmet)
                exec.if(Emmet, function() {
                    dword.setOption('enableEmmet', true);
                }, function(callback) {
                    var url;
        ...
        function loadFiles(callback) {
            exec.series([
                ...
                function(callback) {
                    var css     = PREFIX + '/css/dword.css',
                        js      = '/restafary.js',
                        
                        dir     = DIR + 'codemirror/',
                        theme   = PREFIX + dir + 'theme/pastel-on-dark.css',
                        
                        url     = PREFIX + join([
                            dir + 'addon/mode/loadmode',
                            dir + 'mode/meta'
                        ].map(function(name) {
                            return name + '.js';
                        }));
                        
                    load.parallel([url, theme, js, css], callback);
                },
