...
(function(global) {
    ...
    Dword.prototype._showMessageOnce = once(function(msg) {
    ...
    Dword.prototype._patch = function(path, patch) {
        this._patchHttp(path, patch);
    };
    
    Dword.prototype._write = function(path, result) {
    ...
    Dword.prototype._onSave = function(error, text) {
        ...
        if (error) {
            ...
            smalltalk.confirm(TITLE, msg).then(function() {
                restafary.write(self._FileName, self._Value, self._onSave);
            }).catch(empty).then(function(){
                dword.focus();
            });
        } else {
